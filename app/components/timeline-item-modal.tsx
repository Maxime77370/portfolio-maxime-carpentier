            ...
            return (
                <div className="flex flex-wrap gap-2 mt-2 w-full justify-center">
                    {item[field]?.split(",").map((tech, index) => (
                        <span key={index} className="bg-gray-600 text-gray-200 px-2 py-1 rounded-full text-sm">
                            {tech.trim()}
                        </span>
                    ))}
                </div>
            ...
            return (
                <header className="flex items-center justify-between gap-4 bg-white shadow-md rounded-lg p-4 mt-2">
                    <div className="flex items-center gap-4">
                        {item.type === "project" && <FaInfo className="w-6 h-6 text-gray-800" />}
                        {item.type === "job" && <FaBriefcase className="w-6 h-6 text-gray-800" />}
                        {item.type === "school" && <FaSchool className="w-6 h-6 text-gray-800" />}
...
export const ProjectModal = (props: ModalProps) => (
    <EnhancedModalBase {...props} fields={["title", "description", "images", "link", "technologies"]} />
);
export const JobModal = (props: ModalProps) => (
    <EnhancedModalBase {...props} fields={["title", "images", "description", "mapUrl", "technologies"]} />
);
export const SchoolModal = (props: ModalProps) => (
    <EnhancedModalBase {...props} fields={["title", "images", "description", "location", "mapUrl"]} />
);
